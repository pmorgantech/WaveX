...
//   u  = (x_comp - k S) / (1 + k G^4)               the loop input, solved
//   u  = clip(u)                                    the one nonlinearity
...
class ZdfLadder {
   public:
    ...
    static constexpr float kMaxResonance = 1.25f;
    ...
    // damps it, so the range runs to 1.25 (k = 5) for a clean, bounded
    // self-oscillation; the clip is what bounds it.
    ...
    // Gain into the clip, 0.25 .. 4; the output is scaled back by the same
    // amount so a quiet signal is level-neutral and drive only sets how hot
    // the loop must run before it folds.
    void SetInputDrive(float drive) {
        drive_ = drive < 0.25f ? 0.25f : (drive > 4.0f ? 4.0f : drive);
        drive_inv_ = 1.0f / drive_;
    }
    ...
    float Process(float in) {
        ...
        float u = (in * input_gain_ - k_ * S) * loop_inv_;
        u = Clip(u * drive_) * drive_inv_;
   ...
   private:
    ...
    // The rational tanh the Huovilainen port uses, so the two ladders share
    // a saturation curve and an A/B hears topology, not clipper.
    ...
    float drive_ = 1.0f, drive_inv_ = 1.0f;
