...
static volatile uint32_t
    s_ready_rx_buffers[RX_BUFFER_COUNT];  // Buffer ready flags (0=free, 1=ready)
...
static const uint32_t MIN_POLL_INTERVAL_MS =
    5;  // Increased from 1ms to 5ms for better CPU efficiency
...
static daisy::SpiHandle::Result Spi_SendPacket(const uint8_t* tx_buf, size_t packet_size) {
    ...
    if (dma_result != daisy::SpiHandle::Result::OK) {
        ...
        switch (dma_result) {
            case daisy::SpiHandle::Result::OK:
                error_msg = "Should not be here - OK case";
                break;
            case daisy::SpiHandle::Result::ERR:
                error_msg = "General SPI error";
                break;
            default:
                error_msg = "Unknown result code";
                break;
        }
        if (s_hw)
    ...
    daisy::SpiHandle::Result res = g_spi_handle->BlockingTransmitAndReceive(
        (uint8_t*)tx_buf, rx_buf, packet_size, 10);  // Reduced from 1000ms to 10ms
...
void Spi_Init(daisy::DaisySeed& hw, daisy::SpiHandle* hspi) {
    ...
    cs_pin.Init(
        cs_p, daisy::GPIO::Mode::OUTPUT, daisy::GPIO::Pull::PULLUP, daisy::GPIO::Speed::VERY_HIGH);
    ...
    spi_config.baud_prescaler =
        daisy::SpiHandle::Config::BaudPrescaler::PS_16;  // Slower clock for debugging
    ...
    GPIO_InitStruct.Mode =
        GPIO_MODE_IT_RISING;  // Interrupt on rising edge (ESP32 signals data ready)
...
static void PrepareTxBuffer(uint8_t* tx_buf, size_t buf_size) {
    ...
    if (outgoing_count > 0) {
        ...
        size_t packet_size = WaveX::Protocol::ProtocolHandler::CreatePacket(
            tx_buf,
            buf_size,
            static_cast<WaveX::Protocol::MessageType>(msg_type),
            nullptr,
            0,
            PKT_FLAG_ACK  // ACK flag indicates this is an acknowledgment
        );
...
static bool ProcessReceivedPacket(const uint8_t* rx_buf, size_t transfer_size) {
    ...
    size_t parse_size =
        (expected_size > 0 && expected_size <= transfer_size) ? expected_size : transfer_size;
    ...
    bool parsed_ok = parse_wave_packet(
        rx_buf, parse_size, msg_type, payload, payload_size, sequence_number, flags);
