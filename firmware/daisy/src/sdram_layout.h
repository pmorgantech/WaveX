...
//
// 256 KB since each record gained the sample's card path (sample_pool.hpp):
// records went ~132 B -> ~230 B and no longer fit 192 KB. The 64 KB comes
// out of the render scratch, which shares a fixed 4 MB with this partition,
// so the sample ARENA is unchanged - the one number that would have cost
// user-visible sample memory. Nothing consumes the render scratch yet; when
// an offline render job does, size it against 3.75 MB.
constexpr uint32_t kSampleRegistryBytes = 256u * 1024u;
