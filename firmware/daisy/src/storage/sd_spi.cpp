...
static void clocks(unsigned n) {
    uint8_t ff = 0xFF, rx;
    while (n--)
        s_spi.BlockingTransmitAndReceive(&ff, &rx, 1, 10);
}
...
static uint8_t wait_token(uint32_t ms) {
    ...
    do {
        r = xfer1(0xFF);
    } while (r == 0xFF && (System::GetNow() - start) < ms);
    return r;
}
...
static uint8_t cmd(uint8_t c, uint32_t arg, uint8_t crc) {
    xfer1(0xFF);
    uint8_t frame[6] = {(uint8_t)(0x40 | c),
                        (uint8_t)(arg >> 24),
                        (uint8_t)(arg >> 16),
                        (uint8_t)(arg >> 8),
                        (uint8_t)(arg),
...
bool Init(
    DaisySeed& hw, Pin sck, Pin miso, Pin mosi, Pin cs_pin, SpiHandle::Config::Peripheral periph) {
    s_hw = &hw;
