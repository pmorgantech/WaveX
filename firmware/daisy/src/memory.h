...
class SmallSlabPool {
   public:
    ...
    bool add_pages_for_class(uint8_t cls, uint16_t count) {
        if (cls >= WXM_SMALL_CLASS_COUNT)
            return false;
        if (!ensure_space(count * WXM_SMALL_PAGE_BYTES))
            return false;
    ...
    bool alloc(uint32_t nbytes, wxsamp_t* out) {
        uint8_t cls;
        uint16_t bsize;
        if (!pick_class(nbytes, &cls, &bsize))
            return false;
        ...
        for (uint16_t p = 0; p < pcnt; ++p) {
            ...
            if (pg.used < pg.slots) {
                ...
                if (slot >= 0) {
                    ...
                    stats_.objects_alive++;
                    stats_.in_use_bytes += bsize;
                    return true;
                }
            }
        }
        // Need a new page
        if (!format_new_page(cls))
            return false;
    ...
    void fill_small_stats(wxsamp_stats_t& s) const {
        ...
        for (uint8_t c = 0; c < WXM_SMALL_CLASS_COUNT; ++c) {
            ...
            uint16_t pages = class_page_count_[c];
   ...
   private:
    ...
    bool ensure_space(uint32_t add_bytes) {
        if (used_bytes_ + add_bytes > size_) {
    ...
    bool format_new_page(uint8_t cls) {
        if (class_page_count_[cls] >= WXM_SMALL_MAX_PAGES_PER_CLASS) {
            stats_.failed_allocs++;
            return false;
        }
        if (!ensure_space(WXM_SMALL_PAGE_BYTES))
            return false;
    ...
    static bool mark_slot_free(wxsamp_internal::SlabPage& pg, uint16_t slot) {
        if (slot >= pg.slots)
            return false;
        uint32_t i = slot / 32u;
        uint32_t bit = slot % 32u;
        uint32_t mask = (1u << bit);
        if (pg.bm[i] & mask)
            return false;  // was already free
        pg.bm[i] |= mask;
        return true;
    }

    static void fill_handle_small(
        wxsamp_t& h, uint8_t cls, uint16_t page, uint16_t slot, uint32_t len) {
        h.cls = cls;
        h.page = page;
        h.slot = slot;
        h.len = len;
...
// External definition for static constexpr member (required by some compilers/linkers)
constexpr uint16_t SmallSlabPool::class_block_[WXM_SMALL_CLASS_COUNT];
   ...
   public:
    ...
    bool alloc(uint32_t nbytes, wxsamp_t* out) {
        ...
        if (free_runs_[idx].count == need_pages)
            ...
            free_runs_[idx].first += need_pages;
   ...
   private:
    ...
    int find_best_fit(uint32_t need) {
        ...
        for (int i = 0; i < run_count_; ++i) {
            auto& r = free_runs_[i];
            if (r.count >= need && r.count < best_count) {
                best = i;
                best_count = r.count;
            }
        }
    ...
    void remove_run(int idx) {
        for (int i = idx; i < run_count_ - 1; ++i)
            free_runs_[i] = free_runs_[i + 1];
        run_count_--;
    }

    void insert_run(Run r) {
        if (run_count_ < WXM_LARGE_MAX_RUNS)
            free_runs_[run_count_++] = r;
    }

    void coalesce() {
        // simple insertion sort by first
        for (int i = 1; i < run_count_; ++i) {
            Run key = free_runs_[i];
            int j = i - 1;
            while (j >= 0 && free_runs_[j].first > key.first) {
                free_runs_[j + 1] = free_runs_[j];
                j--;
            }
            free_runs_[j + 1] = key;
        }
        ...
        for (int i = 0; i < run_count_; ++i) {
            if (w == 0) {
                free_runs_[w++] = free_runs_[i];
                continue;
            }
            auto& prev = free_runs_[w - 1];
            auto cur = free_runs_[i];
            if (prev.first + prev.count == cur.first) {
    ...
    uint32_t free_bytes() const {
        uint64_t s = 0;
        for (int i = 0; i < run_count_; ++i)
            s += (uint64_t)free_runs_[i].count * WXM_LARGE_PAGE_BYTES;
        return (uint32_t)s;
    }
    uint32_t largest_free_bytes() const {
        uint32_t m = 0;
        for (int i = 0; i < run_count_; ++i) {
            uint32_t b = free_runs_[i].count * WXM_LARGE_PAGE_BYTES;
            if (b > m)
                m = b;
        }
        return m;
    ...
    static void fill_handle_large(wxsamp_t& h,
                                  uint32_t first_page,
                                  uint16_t page_count,
                                  uint32_t len) {
        h.cls = 0xFF;
...
class SampleMemMgr {
   public:
    ...
    bool prewarm_small(uint8_t cls, uint16_t pages) {
        return small_.add_pages_for_class(cls, pages);
    }

    static constexpr uint32_t class_threshold_bytes() {
        // largest small class size (1KB - last element of {32,64,128,256,512,1024})
        return 1024u;
    }
...
static void wxm_mem_test() {
    ...
    SampleMemMgr mm;
    mm.init(fake_sdram, sizeof(fake_sdram), 256 * 1024);  // 256KB small pool

    // small alloc
    wxsamp_t a{};
    bool ok = mm.alloc(200, &a);
    assert(ok && a.cls != 0xFF);
    void* p = nullptr;
    mm.ptr(a, &p);
    assert(p != nullptr);
    ...
    // large alloc ~200KB
    wxsamp_t b{};
    ok = mm.alloc(200 * 1024, &b);
    assert(ok && b.cls == 0xFF);
