...
void ListenersManager::invoke_wave_chunk_callback(uint32_t offset,
...
void ListenersManager::invoke_sample_status_callback(uint16_t sample_id,
                                                     uint8_t state,
    ...
    if (m_sample_status_cb) {
        m_sample_status_cb(
            sample_id, state, sample_rate, channels, frames_played, m_sample_status_ud);
    }
}
