...
wavex_window_t* wavex_window_create(lv_obj_t* parent, const char* title)
{
    ...
    lv_obj_set_size(window->content, lv_pct(100), lv_pct(100) - UI_HEADER_HEIGHT - UI_HOTKEY_HEIGHT);
...
void wavex_window_set_hotkeys(wavex_window_t* window, const wavex_hotkey_config_t* config)
{
    ...
    for (int i = 0; i < 6; i++) {
        if (config->labels[i] && strlen(config->labels[i]) > 0) {
            ...
            // Add event callback
            lv_obj_add_event_cb(button, config->callback ? config->callback : hotkey_button_event_cb, 
...
{
    if (!window || !window->title_label) return;
