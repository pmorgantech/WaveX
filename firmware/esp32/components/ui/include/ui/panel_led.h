 ...
 * page state in one place - no page sets an LED directly. The PCA9956B channel
 * behind each is the WAVEX_LED_CH_* map in hardware_config.h. The central
 * panel_led_model.h policy publishes complete frames to panel_task.
 */
enum class PanelLed : uint8_t {
    ...
    Soft5,
    Soft6,
    Shift,       ///< physical button held (independent of sticky UI Shift)
...
// Indexed by PanelLed: its PCA9956B channel. The numbers are wiring truth and
...
constexpr uint8_t kLedChannels[kPanelLedCount] = {
    ...
    WAVEX_LED_CH_SOFT6,
    WAVEX_LED_CH_SHIFT,
    WAVEX_LED_CH_JUMP_SAMPLE,
    WAVEX_LED_CH_JUMP_PLAY,
    WAVEX_LED_CH_JUMP_INSTRUMENT,
    WAVEX_LED_CH_JUMP_TRACK,
    WAVEX_LED_CH_JUMP_MIXER,
    WAVEX_LED_CH_JUMP_SETTINGS,
    WAVEX_LED_CH_PLAY_STOP,
    WAVEX_LED_CH_REC,
    WAVEX_LED_CH_PAD1,
    WAVEX_LED_CH_PAD2,
    WAVEX_LED_CH_PAD3,
    ...
    WAVEX_LED_CH_PAD15,
    WAVEX_LED_CH_PAD16,
    WAVEX_LED_CH_JUMP_SEQUENCER,
    WAVEX_LED_CH_JUMP_DIAGNOSTICS,
};
...
static_assert(ledChannelsUnique(), "two PanelLeds share a PCA9956B channel (hardware_config.h)");

}  // namespace detail

/// PCA9956B channel index (0 .. WAVEX_LED_CHANNELS-1) of a panel LED.
