...
#define WAVEX_UART_DEBUG_LEVEL 1  // 1 = errors only; raise toward UART_LOG_DUMP (5) for more
#endif
...
#if WAVEX_UART_DEBUG_LEVEL >= UART_LOG_ERROR
#define UART_LOGE(tag, fmt, ...)                                                               \
    do {                                                                                       \
        printf("[UART][E]%s%s: " fmt "\n", tag ? " " : "", tag ? tag : "uart", ##__VA_ARGS__); \
    } while (0)
#else
#define UART_LOGE(tag, fmt, ...) ((void)0)
#endif
...
#if WAVEX_UART_DEBUG_LEVEL >= UART_LOG_WARN
#define UART_LOGW(tag, fmt, ...)                                                               \
    do {                                                                                       \
        printf("[UART][W]%s%s: " fmt "\n", tag ? " " : "", tag ? tag : "uart", ##__VA_ARGS__); \
    } while (0)
#else
#define UART_LOGW(tag, fmt, ...) ((void)0)
#endif
...
#if WAVEX_UART_DEBUG_LEVEL >= UART_LOG_INFO
#define UART_LOGI(tag, fmt, ...)                                                               \
    do {                                                                                       \
        printf("[UART][I]%s%s: " fmt "\n", tag ? " " : "", tag ? tag : "uart", ##__VA_ARGS__); \
    } while (0)
#else
#define UART_LOGI(tag, fmt, ...) ((void)0)
#endif
...
#if WAVEX_UART_DEBUG_LEVEL >= UART_LOG_VERBOSE
#define UART_LOGV(tag, fmt, ...)                                                               \
    do {                                                                                       \
        printf("[UART][V]%s%s: " fmt "\n", tag ? " " : "", tag ? tag : "uart", ##__VA_ARGS__); \
    } while (0)
#else
#define UART_LOGV(tag, fmt, ...) ((void)0)
#endif
...
#else
#define UART_LOG_DUMP_PACKET(tag, data, len) \
    do {                                     \
        (void)(tag);                         \
        (void)(data);                        \
        (void)(len);                         \
