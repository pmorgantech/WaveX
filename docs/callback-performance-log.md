...
underruns/drops, 386 live-edit cycles and five file cycles. It is REVIEW and
blocks further callback features until the callback cost is reduced; it does
not close the one-hour phase soak. The capture used the clean `328a419` source
and `build-profile-held` image SHA256
