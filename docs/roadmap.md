...
| Ladder filter budget (measured 2026-09-14, decision open) | `scripts/bench_filter_topology.py` A/B on the profiling image, 200 s each, eight held looped voices, 24 dB, full drive, slow triangle voice LFO and long Env 2/3 into cutoff and resonance, cutoff updates every 1.2 s, no sequencer or SD stream: **SVF peak 30.25%, ladder peak 69.82%**, zero underruns both (`logs/perf-svf-mod-20260914-023223.log`, `logs/perf-ladder-mod-20260914-022750.log`; the helper refuses captures under 600 s, so these are A/B rows, not gate rows). The ladder therefore costs ~40 points of the block budget for eight voices, ~5 per voice; on the full 2026-09-12 workload, where the SVF alone sits at 69.9%, eight ladder voices would exceed the budget. The parameter stays; the 4x ladder cannot be the eight-voice default. The A/B set is now on the MODEL tile: `Ladder` (4x, the measured one), `Ladder 2x` (same port at Huovilainen's published rate, about half the cost) and `ZDF` (first-party zero-delay-feedback ladder, about the SVF's cost). Capture `ladder2x` and `zdf` with the same script, listen to all three against each other (LP/HP/BP at both slopes, Notch as input minus the 12 dB band-pass tap, resonance, drive), then prune: the wire values stay reserved for whatever is withdrawn. |
