...
| Ladder filter budget (measured 2026-09-14, decision open) | `scripts/bench_filter_topology.py` A/B on the profiling image, 200 s each, eight held looped voices, 24 dB, full drive, slow triangle voice LFO and long Env 2/3 into cutoff and resonance, cutoff updates every 1.2 s, no sequencer or SD stream: **SVF peak 30.25%, ladder peak 69.82%**, zero underruns both (`logs/perf-svf-mod-20260914-023223.log`, `logs/perf-ladder-mod-20260914-022750.log`; the helper refuses captures under 600 s, so these are A/B rows, not gate rows). The ladder therefore costs ~40 points of the block budget for eight voices, ~5 per voice; on the full 2026-09-12 workload, where the SVF alone sits at 69.9%, eight ladder voices would exceed the budget. The parameter stays; the ladder as vendored cannot be the eight-voice default. Decide between: a first-party copy of the ladder at 2x rather than 4x oversampling (about half the cost, still too much for all eight on the full workload), the Stilson model (no oversampling, an order cheaper, weaker resonance behaviour), or a per-topology voice cap. Measure the replacement with the same script before choosing. Listening: not yet judged by ear (LP/HP/BP at both slopes, Notch as input minus the 12 dB band-pass tap). |
