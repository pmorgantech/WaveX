...
|---|---|---|
| ~~Note-on still hardcodes MIDI root note 60 instead of using the built instrument resolver~~ **Restated 2026-09-02**: it does not. The instrument path is wired and resolves root note per zone; a hardcoded 60 remains only on the no-instrument-bound fallback that plays a bare loaded WAV. **Updated 2026-09-02**: the fallback's *any-channel* behaviour is retired — bare-WAV playback is now per-slot (`MSG_SAMPLE_SELECT{sample_id, slot}`), so a slot with nothing bound just drops the note instead of guessing. `OnNoteOn` still has two branches (SFZ-instrument-bound vs. bare-WAV), not the one-zone-instrument unification this row originally proposed — see below. **Closed 2026-09-02**: the branches are unified — a bare binding is a one-zone `Instrument` (`SfzLoader::BindSample`) resolved through the same bank path as an import, and the root note is that zone's property. | Phase 2.5 item 1 (done) | One `OnNoteOn` path; no hardcoded root note remains anywhere in the note path |
...
> **Active work order: [`features/digital-voice-audition.md`](features/digital-voice-audition.md).** The consolidated, ordered path to a playable and sequenceable **all-digital** voice. It exists because this work was spread across five documents interleaved with work the goal does not need. Stages 1, 3 and 4 (Goal A: live digital voice parameters, the grid page, live parameter editing) are now done — `ui_play_page.cpp` sends `MSG_NOTE_ON`/`OFF` and `MSG_CONTROL_CHANGE`, and `OnControlChange` publishes both the analog `s_para_pending` and digital `s_voice_live_pending` state for the callback. **Stage 2 is narrower than this document long claimed** (corrected 2026-09-02 by reading `OnNoteOn`): the resolver path *is* wired. A note on a slot with an instrument bound goes `SfzLoader::SlotLoaded()` → `ResolveNote()` → `ResolveNoteOn()`, with zone matching, velocity layers, per-zone root note and tuning fold all applied. `kDefaultRootNote` survives only on the **fallback** branch, taken when no instrument is bound and the note plays the most recently loaded bare WAV. So what is open is not "use the resolver" but "stop having a second path at all" — see Phase 2.5 item 1. The remaining Stage 2b UI diagnostics are still a Goal A gap, and Goal B (the sequencer path, stages 5–8) has not started. This is the active remediation for Principles 2, 3, and 11.
...
1. **Instrument model, remaining pieces** (`features/instrument-model.md`). **Most of this list was already built when it was written** — verified against the source 2026-09-02:
   - ~~the sample table that populates a `SampleResolver` from loaded WAVs~~ — **built**, as `Sfz::SampleTable` in `audio/sfz_import.hpp`. Note `instrument.hpp`'s own header comment still says this is "deliberately NOT here… lives in audio_engine.cpp (stage 4)", which is now wrong twice over: it exists, and it lives somewhere else again.
   - ~~the `MSG_INST_OP/STATUS` protocol (0x60–0x61)~~ — **built** and dispatched; the browser drives probe and load through it (SFZ probe/load and, since, `SET_MOD_SLOT`). `MSG_INST_ZONE_SYNC` (0x62) is still reserved and unused. **Restated 2026-09-02**: editable zones now ARE designed — `features/instrument-model.md` §12 lays out a pad→sample mapping op and a bank-management (save/browse/load) plan, in response to a direct user request. Not started.
   - ~~the ESP32 UI~~ — **partly built**: the sample browser lists `.sfz`, preflights referenced WAVs and resident memory, and loads. What is missing is any way to *see or edit* an instrument once loaded.
   - ~~The bare-WAV note-on fallback~~ — **done (2026-09-02), in two steps.** First the fallback's any-channel behaviour was retired: `MSG_SAMPLE_SELECT{sample_id, slot}` binds a sample to one of the 16 slots, and an unbound slot drops the note (ESP32 UI: the Play page's Slot parameter, the Sample Manager's Select + Slot -/+, the Voice page's SAMPLE/SLOT controls). Then the two `OnNoteOn` branches were unified: a bare sample bound to a slot is now a one-zone Keyboard `Instrument` in `SfzLoader`'s bank (`SfzLoader::BindSample()`, origin `Built`) and resolves through exactly the `InstrumentBank::ResolveNote()` path an `.sfz` import uses — `OnNoteOn` has one path and no `kDefaultRootNote`; the root note is the zone's. What the old branch did by hand is now zone/resolver state: `SampleRef` carries the sample record's resolved markers, fades and gain, which a zone inherits wherever it leaves its own field at 0 (`instrument-model.md` §2's rule, finally implemented; `Zone::loop_mode` gained an explicit *Off* distinct from *Inherit*), and `ZONE_FLAG_LIVE_FILTER_ENV` makes a zone take filter/ADSR from `VoiceLiveParams` at trigger, so a note after a knob move still sounds like the sweep. `Instrument::origin` records which sample-id registry a zone's ids index (the SFZ table vs. the WAV registry — the two overlap), which is the `SampleResolver` decision `instrument-model.md` §12.1 deferred; the engine registers a bridging resolver over its registry via `SfzLoader::SetLoadedSampleResolver()`. Two behaviour changes worth knowing: note-offs on a bare-bound slot are now slot-scoped like an import's (a note-off on slot 3 no longer releases slot 5's same-numbered note), and the SFZ-replacement reset is scoped to the slot being replaced, so bare bindings (and mod slots) on other slots survive an import. One deliberate refusal: binding a bare sample over a slot that holds an SFZ import is rejected with a log line rather than silently shadowed — its samples can only be released through the load handshake, which is [the single-residency limit](backlog.md#only-one-instrument-slot-can-be-resident-at-a-time), still a separate project. Not yet verified on hardware.
   - **Open**: the callback wiring shared with Phase 2 item 1, and hardware audition.
2. **Mixer v1** (`features/output-routing-and-mixer.md` §1–2): 16-track gain/pan/mute/solo + per-track meters (0x78/0x79). Small, and the performance work below wants it. Staged per that doc's §6:
   - ~~Stage 1: the `TrackMix` model~~ — **done**, `firmware/shared/audio/track_mix.hpp` with 22 host tests. Shared rather than Daisy-local because the two sides own different halves and must agree on the arithmetic: the Daisy holds the table, the ESP32 owns solo (expanded to mutes before sending, so a dropped link cannot strand a hidden solo), and both convert dB↔linear and peak↔meter-byte. **Engine application is deliberately not in this stage** — the design's intended seam is the block-modulation surface from item 4, which does not exist yet, and wiring an inert table into the audio callback before anything can drive it adds unverified callback risk for no observable behaviour. It lands with stage 2.
   - ~~Stage 2: protocol `MSG_MIX_OP` (0x78) / `MSG_MIX_METERS` (0x79) + round-trip tests + doc rows~~ — **done**. Encodings documented per op, converted through `WaveX::Mix` so both ends share one implementation, and pan reuses `PARAM_PAN`'s convention rather than inventing a second. Added `SET_MUTE_MASK` beyond the design's op list: expanding a solo into up to 16 separate mute messages walks the engine through intermediate states that the 5 ms mute ramps make audible.
   - ~~Stage 2b: **engine application**~~ — **done**. `VoiceManager::SetTrackMixer()` folds track gain and pan offset into the voice's existing `left_gain`/`right_gain`, computed once per voice per block *outside* the sample loop, so the mixer costs two multiplies and an add per voice per block and nothing per sample — exactly the application point the design specifies. A null mixer reproduces the previous behaviour byte for byte, which is what lets the existing voice tests keep asserting an unmixed sum. `MIX_OP_SET_MASTER` is stored but **not applied**: `PARAM_VOLUME` still owns master gain, and two controls writing one value with no defined winner is worse than one that waits for the mixer page to reconcile them.
...
4. **Modulation matrix + LFOs + filter envelope** (`features/param-locks-and-modulation.md` §3–5): 8 slots/instrument, block-rate evaluation, `MSG_MIDI_CC` (0x56). Land the param slew engine (`features/scenes-and-performance.md` §3) here — same control-tick surface. Staged:
   ...
   - ~~The **mod matrix** itself (`ModSlot`, sources, curves, per-tick evaluation)~~ — **done**, `audio/mod_matrix.hpp`, 19 host tests. Pure: it reads a source snapshot and returns block-rate multipliers, so the caller decides when per-trigger sources are sampled. Slots on one destination sum rather than overwrite. Two ranges had to be **chosen rather than derived**, since the design fixes the evaluation model but not what 100% depth should move — 4 octaves of cutoff and 2 semitones of pitch, named constants so they can be tuned by ear.
   - ~~`Voice::SetBlockModulation()` and the control-tick evaluation~~ — **done**. `VoiceManager::TickModulation()` runs once per callback (one callback IS one 1kHz control tick, `timebase.hpp`), ticks the two global LFOs, evaluates `EvaluateModMatrix()` per sounding voice against **that voice's own instrument's slots** and writes the result via `Voice::SetBlockModulation()`; `Render()` applies pitch/cutoff (gated on `!= 1.0` so an unmodulated voice pays neither a `tan()` recompute nor an increment rewrite) and gain/pan (unconditional, one multiply/add) once per voice per block. 9 host tests extending `VoiceManagerTest`.
   - ~~The **second envelope** as `SRC_ENV_FILTER`~~ — **done**. `Voice::env2` (same `Envelope` class as the amp envelope), triggered/released alongside `envelope` from `Trigger()`/`Release()`/`ReleaseSlot()`/`Choke()`. Evaluated once per block via the new `Envelope::AdvanceBlock(n)` rather than `Process()` in a per-sample loop — same result, without doubling the amp envelope's per-sample cost for a source nothing reads between control ticks. ADSR comes from new `VoiceTriggerParams` fields (`filter_env_*`), not yet zone-derived (that needs the wire-Zone chunk bump below). 10 host tests (6 for `AdvanceBlock` itself, 4 extending `VoiceManagerTest`/`VoiceManagerModulationTest`).
   - ~~Instrument-scoped `ModSlot` storage + the mod-slot protocol op~~ — **done**. `Instrument::mod_slots[8]` (`instrument.hpp`), written by a new `INST_OP_SET_MOD_SLOT` op riding `MSG_INST_OP` (extends `InstOpMessage`, `path` unused for this op) via `SfzLoader::SetModSlot()`; `TickModulation()` looks each voice's own instrument up through a `ModSlotResolver` (function pointer + context, mirroring `instrument.hpp`'s `SampleResolver`, so `VoiceManager` stays independent of the instrument model above it). ESP32 send wrapper `inter_mcu_send_mod_slot()`; round-trip + dispatch tests; `inter-mcu-protocol.md` updated; `PARAM_MODULATION_MATRIX` (0x0A)'s `OnControlChange` case retired (enum value kept, reserved). No UI sends this op yet (stage 5 below), so it is still a no-op on hardware until something does.
   - Open: a per-voice LFO as `SRC_LFO_VOICE` — still evaluates to 0 (`ModSources`' default), the same treatment `SRC_PARA_ENV` already gets for the deferred analog stage.
   - Open: giving `Zone` its own `SRC_ENV_FILTER` ADSR (wire-Zone chunk v2, append fields + bump version) — every trigger currently gets the same default shape regardless of instrument.
   - Open: `MSG_MIDI_CC` (0x56) itself is fully wired (struct, dispatch, tests) and was already before this stage, but two things around it are not — the ESP32 MIDI task drops incoming CC/channel-pressure instead of forwarding them (channel pressure isn't even a parsed event in the shared MIDI stream parser yet), and the Daisy doesn't feed a received CC into `ModSources.modwheel`/`.aftertouch` (`SequencerTransport::OnMidiCc()` only records it for tests). `SRC_MODWHEEL`/`SRC_AFTERTOUCH` read 0 until that's built.
