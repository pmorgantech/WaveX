...
1. **Instrument model, remaining pieces** (`features/instrument-model.md`). **Most of this list was already built when it was written** — verified against the source 2026-09-02:
   ...
   - ~~the `MSG_INST_OP/STATUS` protocol (0x60–0x61)~~ — **built** and dispatched; the browser drives probe and load through it. `MSG_INST_ZONE_SYNC` (0x62) is still reserved and unused, which is correct — editable zones are not designed yet.
