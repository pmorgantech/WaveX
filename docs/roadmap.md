...
3. **`esp_spi_link.cpp` has two unresolved findings from a 2026-08-30 ESP32 coding-guide review**, found while it is compiled out — both need to be fixed before item 2 is ever picked up, not blindly patched now:
   - **`spi_slave_task()`'s buffer pool can be handed back to the driver while still queued.** Every loop iteration advances the TX/RX buffer-pool index and overwrites `s_transactions[idx]`/`s_rx_buffers[idx]`/`s_tx_buffers[idx]` unconditionally, without confirming `spi_slave_get_trans_result()` actually returned *that* slot's prior transaction. `BUFFER_POOL_SIZE` is 3, matching the driver's `queue_size`; if the peer stalls for ~3 iterations (the RX wait is a 50 ms timeout treated as normal, not exceptional) with all three slots still outstanding, the fourth iteration mutates a descriptor and DMA buffer the driver may still hold live — guide §7 ("never modify a buffer while hardware is transmitting from it"). Needs a real ownership-tracking redesign (a `FREE`/`DMA_ACTIVE`/... state per slot, per guide §7/§10), not a one-line fix.
   - **`spi_post_trans_cb` is registered as the SPI slave driver's ISR callback and calls `gpio_set_level()` and `AttnWatchdog::MarkCleared()`, neither audited for IRAM-safety.** Guide §4: an ISR's entire transitive call/data graph must be IRAM/DRAM-resident before it can be trusted from an `ESP_INTR_FLAG_IRAM` context. This driver instance doesn't request that flag today, so it likely works by accident rather than by audit — confirm the flag configuration and audit the call graph before relying on it, rather than assuming flash-resident code is safe here.
