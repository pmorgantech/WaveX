...
| Live voice-parameter path | `MSG_CONTROL_CHANGE` now reaches the digital voices (`s_voice_live_params` → `ApplyLiveParams`), so a knob should be audible on notes already sounding. The whole chain is host-tested but has never run on hardware, and the block-rate update cadence is exactly where zipper noise would show up if it is going to. | Hold a note, sweep cutoff and resonance: it should be smooth and continuous, not stepped. Then release and re-trigger — the new note must start where the sweep left it, not jump back. |
