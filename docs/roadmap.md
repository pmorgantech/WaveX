...
| Stacked L/R waveform traces | The split, the divider, the per-lane grid and the L/R labels are all geometry, and geometry is exactly what a host test cannot judge — `WaveformView` is not in the host test library (no LVGL on the host), so this landed with **no automated coverage at all**. The specific risk is a silent channel swap: L and R are distinguishable only by which lane they land in, and the de-interleave (`columns[idx * stride + ch]`) was verified by reading `EnvelopeCache::render()`, not by running it. Halving each trace's height is also an unjudged legibility change on a real panel. | Open Sample Edit on a **stereo** WAV whose channels plainly differ — easiest is one with content hard-panned to one side. The panel must split, label the lanes L and R, and the loud channel must appear in the lane its label names. Confirm the divider reads as a boundary rather than a grid line, and that each trace still has a visible zero line. Then open a **mono** sample: one full-height trace, no labels, no divider. Finally check first-frame render cost on entry has not regressed (`scripts/sysmon_stats.py`, cf. `docs/backlog.md`) — this doubles the stored columns and can double the span draw tasks. |
