...
2. **Drop shadow on the card/tab chrome** — **applied 2026-08-30**, build-verified only. 9.5 added native software blur and drop shadow (`lv_style_set_drop_shadow_color/offset_x/offset_y/opa/quality/radius`, `lv_style_set_blur_radius/quality/backdrop`). These are *distinct* from the pre-existing `lv_style_set_shadow_*` properties, and they are **not** behind an `lv_conf` flag — they are already compiled into every image we build, so the only cost of using them is render time. Nothing in `components/ui` calls any shadow API today and `ui_palette.h` has no shadow entry, so this is a live design decision for the chrome added in `6dbf399`, taken while that work is fresh rather than rediscovered later. Render cost on a rotated 1280x720 panel is the thing to watch.

   As built: `ui_palette.h` gained `kShadow*` (radius 8, offset y 3, opa 160, black), and `wavex_ui::cardApplyDropShadow()` in `src/ui_card.cpp` applies them. The helper is deliberately additive — it sets only the shadow properties and leaves fill, border, radius and padding alone — so it drops onto an existing card without restyling it, and the values live in one place rather than being copied per page. Applied at four sites: the diagnostics stat card and link message-count panel, and the voice page's chain tiles and parameter panel.

   Two things worth knowing before judging it:

   - **The renderer was already in the image, so this cost almost nothing.** Enabling it added 256 bytes. That is not a surprise once you read `lv_draw_rect.c:78`: the drop-shadow path is guarded by a runtime `if(dsc->base.drop_shadow_opa)` inside the always-linked rect draw, so `--gc-sections` could never have dropped it. We were already paying the flash for blur; we just were not asking for it.
   - **The runtime cost is not free, and it is per-object per-redraw.** Each shadowed object makes `lv_draw_rect` call `lv_draw_layer_create_drop_shadow()`, which allocates a layer and runs a blur pass. `cardApplyDropShadow()` therefore sets `LV_BLUR_QUALITY_SPEED` rather than leaving the quality `AUTO`, since AUTO may pick the precise kernel and this runs continuously rather than once. The diagnostics tab shadows several cards at once, which is the worst case we have.

   **No host preview is possible for this.** `tools/ui_preview` compiles only `preview.c` plus the LVGL sources — it does not build `components/ui` — so it renders its own mock screens, not our real cards. Judging whether the shadow reads at all against a black background needs the panel.
...
| LVGL 9.5 performance claim | The upgrade was justified partly on speed; nothing has measured it. And the flag it actually turned on (`LVGL_PORT_ENABLE_PPA`) accelerates *rotation*, not drawing — so the "~30% faster" rationale was never about the change that was made. [§ 0.3](#03-lvgl-95-take-up-added-2026-08-30) item 1 is the lever that would make it true. | FPS + UI-task CPU before/after on the waveform-preview and meter pages, per `docs/performance_monitoring.md`. Measure `CONFIG_LV_USE_PPA` on/off in the same session as the rotation check above — the two PPA consumers share hardware, so testing them apart is the only way to attribute a bad frame. |
| `CONFIG_LV_USE_PPA` (LVGL PPA draw unit) | Enabled 2026-08-30, build-verified only. Two things could go wrong and they look the same on the panel: the draw unit itself misrenders unrounded opaque fills, or the global full-buffer `esp_cache_msync` it installs (see [§ 0.3](#03-lvgl-95-take-up-added-2026-08-30) item 1) costs more than it saves. It also stacks a second PPA consumer on top of the rotation path in the row above, which has never been checked on hardware either. | Flash and look for wrong or missing background fills first — that is the correctness question. Then FPS + UI-task CPU with `CONFIG_LV_USE_PPA` on vs off, holding `CONFIG_LVGL_PORT_ENABLE_PPA` fixed, so the two PPA consumers are attributable. Revert `CONFIG_LV_USE_PPA`, `CONFIG_LV_PPA_BURST_LENGTH` and `CONFIG_LV_DRAW_BUF_ALIGN` (back to 4) in **both** `sdkconfig` and `sdkconfig.defaults` if it misbehaves — the alignment change is not optional, the build hard-`#error`s without it. |
| Card drop shadows | Applied 2026-08-30, build-verified only, and **not previewable on the host** — `tools/ui_preview` does not compile `components/ui`. Two open questions: whether the shadow reads at all (against `kColBg` black, a black shadow can only darken what is already black, so only the lit card edges can show separation), and what it costs (one `lv_draw_layer_create_drop_shadow()` layer allocation plus a blur pass per shadowed object per redraw). | Flash and look at the diagnostics tab, which shadows several cards at once — the worst case we have — and the voice page chain tiles. If it does not read, raise `kShadowOpa`/`kShadowOffsetY` in `ui_palette.h`, or drop the idea; if it costs, FPS + UI-task CPU with the four `cardApplyDropShadow()` calls commented out. Both knobs are in one place by design. |
