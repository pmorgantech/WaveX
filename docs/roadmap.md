...
4. **Modulation matrix + LFOs + filter envelope** (`features/param-locks-and-modulation.md` §3–5): 8 slots/instrument, block-rate evaluation, `MSG_MIDI_CC` (0x56). Land the param slew engine (`features/scenes-and-performance.md` §3) here — same control-tick surface. Staged:
   ...
   - ~~`Voice::SetBlockModulation()` and the control-tick evaluation~~ — **done**. `VoiceManager::TickModulation()` runs once per callback (one callback IS one 1kHz control tick, `timebase.hpp`), ticks the two global LFOs, evaluates `EvaluateModMatrix()` per sounding voice and writes the result via `Voice::SetBlockModulation()`; `Render()` applies pitch/cutoff (gated on `!= 1.0` so an unmodulated voice pays neither a `tan()` recompute nor an increment rewrite) and gain/pan (unconditional, one multiply/add) once per voice per block. The mod-slot array is engine-global and currently empty (no protocol op writes it yet — see below), so this is a no-op on hardware today: every voice renders exactly as before. 9 host tests extending `VoiceManagerTest`.
   ...
   - Open: the mod-slot protocol op (`SET_MOD_SLOT` on `MSG_INST_OP`) and instrument-scoped slot storage — until it lands, `s_mod_slots` is a fixed engine-global array nothing can write.
