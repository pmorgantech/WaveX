...
1. **Instrument model, remaining pieces** (`features/instrument-model.md`). **Most of this list was already built when it was written** — verified against the source 2026-09-02:
   ...
   - **Partly done (2026-09-02): the fallback's any-channel behaviour is retired, the two-branch structure is not yet unified.** `OnNoteOn`'s bare-WAV path used to resolve to "whatever was most recently loaded, on any MIDI channel" (`s_selected_sample_id`, engine-global). It is now per-slot: `MSG_SAMPLE_SELECT{sample_id, slot}` binds a sample to one of the 16 slots (0..15, same numbering as an instrument slot), `OnNoteOn` looks up the note's own channel's binding, and an unbound slot drops the note rather than falling back to any other channel's sample. ESP32 UI: the Play page's Slot parameter picks which channel notes go out on; the Sample Manager page's Select action (with its own Slot -/+ shifted softkeys) binds a resident sample to a chosen slot; the Voice page's Sample tab gained SAMPLE (cycle resident samples) and SLOT controls. **Still open, and now the harder remaining piece**: unifying the two `OnNoteOn` branches (SFZ-instrument-bound vs. bare-WAV) via a synthesised one-zone `Instrument`, so the root note becomes an editable zone property rather than a constant in the note path, and so bare-WAV playback goes through the SAME `InstrumentBank`/`SampleResolver` machinery SFZ instruments already use. Carry the existing fallback's behaviour across: it applies `s_voice_live_pending` filter/envelope and the sample's `SampleMetadata` markers and gain, none of which a zone currently carries from that source. Also note [the single-instrument-residency limit](backlog.md#only-one-instrument-slot-can-be-resident-at-a-time) found while scoping this: unifying the branches does not by itself make more than one instrument slot resident at once — that is a separate, larger project, tracked in the backlog rather than here.
