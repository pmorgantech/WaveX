...
| Stacked L/R waveform traces | Landed with **no automated coverage**, and the risk that carries is a silent channel swap: L and R are distinguishable only by which lane they land in, and the de-interleave (`columns[idx * stride + ch]`) was verified by *reading* `EnvelopeCache::render()`, not by running it. Halving each trace's height is also an unjudged legibility change on a real panel. **Note the coverage gap is not a hard constraint** — see the correction in `backlog.md` § *A host pixel test for LVGL widgets is possible today*; a hard-panned stereo fixture rendered off-device would settle the swap question without hardware, and only the panel legibility genuinely needs the bench. | Open Sample Edit on a **stereo** WAV whose channels plainly differ — easiest is one with content hard-panned to one side. The panel must split, label the lanes L and R, and the loud channel must appear in the lane its label names. Confirm the divider reads as a boundary rather than a grid line, and that each trace still has a visible zero line. Then open a **mono** sample: one full-height trace, no labels, no divider. Finally check first-frame render cost on entry has not regressed (`scripts/sysmon_stats.py`, cf. `docs/backlog.md`) — this doubles the stored columns and can double the span draw tasks. |
