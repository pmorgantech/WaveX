...
|---|---|---|
| ~~Note-on still hardcodes MIDI root note 60 instead of using the built instrument resolver~~ **Restated 2026-09-02**: it does not. The instrument path is wired and resolves root note per zone; a hardcoded 60 remains only on the no-instrument-bound fallback that plays a bare loaded WAV | Phase 2.5 item 1 (retire the fallback by giving a bare WAV a one-zone instrument) | `OnNoteOn` has exactly one resolution path; the bare-WAV case reaches it through a synthesised zone rather than a parallel branch |
...
1. **Instrument model, remaining pieces** (`features/instrument-model.md`). **Most of this list was already built when it was written** — verified against the source 2026-09-02:
   ...
   - **Open: delete the Phase-1 stopgap note→sample policy.** The honest shape of this is not "delete" — the fallback is what makes "load a WAV in the browser, then press a pad" work, so removing it is a regression unless something replaces it. Give a bare loaded WAV a **synthesised one-zone instrument** instead, so `OnNoteOn` has one resolution path and the root note becomes an editable zone property rather than a constant in the note path. Carry the existing fallback's behaviour across: it applies `s_voice_live_pending` filter/envelope and the sample's `SampleMetadata` markers and gain, none of which a zone currently carries from that source.
