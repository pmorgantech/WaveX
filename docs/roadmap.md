...
| Ladder filter budget (measured 2026-09-14, decision open) | `scripts/bench_filter_topology.py` A/B on the profiling image, 200 s each, eight held looped voices, 24 dB, full drive, slow triangle voice LFO and long Env 2/3 into cutoff and resonance, cutoff updates every 1.2 s, no sequencer or SD stream: **SVF peak 30.25%, ladder peak 69.82%**, zero underruns both (`logs/perf-svf-mod-20260914-023223.log`, `logs/perf-ladder-mod-20260914-022750.log`; the helper refuses captures under 600 s, so these are A/B rows, not gate rows). The ladder therefore costs ~40 points of the block budget for eight voices, ~5 per voice; on the full 2026-09-12 workload, where the SVF alone sits at 69.9%, eight ladder voices would exceed the budget. The A/B set is on the MODEL tile and all three ladders are measured on the same workload (`callback-performance-log.md` § Filter topology A/B): `Ladder` 4x peak 69.8%, `Ladder 2x` 47.5%, `ZDF` 33.7%, against the SVF's 30.3%. The ZDF ladder costs about 3 points for eight voices, so it can be an eight-voice default on the full workload; the 4x cannot and the 2x is marginal. What remains is the listening: all three against each other (LP/HP/BP at both slopes, Notch as input minus the 12 dB band-pass tap, resonance, drive), then prune. Wire values stay reserved for whatever is withdrawn. The Filter tab as built: `logs/filter-tab-20260914.png`. |
