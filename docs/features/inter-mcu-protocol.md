...
| MSG_SAMPLE_SELECT | 0x45 | E→D | `SampleSelectMessage{sample_id}` | selects which loaded sample subsequent `MSG_NOTE_ON` events address; `sample_id` 0 = the most recently loaded playable sample (the pre-existing behaviour) |
