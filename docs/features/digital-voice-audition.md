...
### Stage 1 — Live digital voice parameters

The load-bearing stage. Three parts:

1. **A per-slot base-parameter record** in the engine (cutoff, resonance, A/D/S/R), written by `OnControlChange` and read when building `VoiceTriggerParams`. This alone makes the *next* note reflect an edit.
2. **A live-update surface on `VoiceManager`** so a *sounding* voice follows edits — the parameter sweep is the point, and a filter that only changes between notes cannot be swept. Shaped as a block-rate write, matching the surface `param-locks-and-modulation.md` §3 already specifies (`SetBlockModulation`-style: one struct write, callback-safe, applied at the top of the render slice). Building it to that shape now means the mod matrix later drives an existing surface rather than inventing a second one.
