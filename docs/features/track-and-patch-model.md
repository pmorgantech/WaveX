...
End state: **one registry, refcounted by path** — the Sample Pool the UI names — exactly as `instrument-model.md` §4 specified and never built:
...
1. **Rename** — UI strings and docs: Slot → Track, Voice page → Instrument, "Patch" → Instrument wherever a string still says it. Code identifiers in a second, purely mechanical commit (`kNumInstrumentSlots → kNumTracks`, `InstrumentBank → Tracks`, `Slot() → Track()`, `Voice::slot → Voice::track`, `NoteMessage::channel` documented as track-or-channel, `kNumVoices → WAVEX_NUM_VOICES`). No behaviour change; builds and tests only.
