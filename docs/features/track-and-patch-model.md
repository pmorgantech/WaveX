...
enum class FilterTopology : uint8_t { Svf = 0, Ladder, LadderLite, LadderZdf /* which implementation renders the type */ };
...
| **Sample Browser** | what is on the card; audition; **Load onto the selected Track** (§6.1) | WAV-only listing; Load binds to the selected Track |
...
### Keyboard editor verification — 2026-09-11

The Key Map exposes the existing single-oscillator, keyboard-mode Instrument:
32 stable zone slots, inclusive key/velocity limits and root notes. Overlaps use
the existing ordered resolver and four-layer trigger cap. Edits preserve the
other zone fields; assignment resets region/loop markers when the sample changes.

Host tests cover wire round trips, stale revisions, the Track-local sample
retirement barrier, pool references, immutable prepared triggers and sparse WXI
round trips. Two-board HIL covers boundary velocities, staged Apply/Revert,
held-note continuity, WXI Browser recall, clearing a copied zone without changing
the original, and sequencer-grid velocity selection. Track binding now identifies
only an unnamed full-range single-zone Quick Instrument as a bare sample; named,
sparse and split maps remain Instrument bindings.
