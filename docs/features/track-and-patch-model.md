...
4. **Instrument file and editors** — in progress. Done: Instrument-level defaults + zone overrides (§3.2), with parameter changes addressed to a Track. Remaining: `.wxi` reader/writer with the full §3.3 chunk set (osc2/env3/LFO chunks may be written empty until stage 5), `INST_OP_NEW/SAVE/SET_NAME/SET_PAD_SAMPLE/SET_ZONE`, Instrument Browser, Pad Map and Key Map pages, Track page. The Pad Map piece does not depend on stage 3 and may go first.
