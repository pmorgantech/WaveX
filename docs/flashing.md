...
- Connect the target board by USB and confirm it is passed through to the container. The default ESP32 serial port is `/dev/ttyACM0`; check the actual device name after connecting it:
...
This uses `/dev/ttyACM0` at 2,000,000 baud. If the board appears on another port, run the equivalent ESP-IDF command from `firmware/esp32` with that port instead:
...
If the ESP32 command cannot find `/dev/ttyACM0`, reconnect the board and use the detected `/dev/ttyACM*` or `/dev/ttyUSB*` port with the direct `idf.py` command above. If the port is visible on the host but not inside the container, reopen the devcontainer so its USB device mapping is refreshed.
