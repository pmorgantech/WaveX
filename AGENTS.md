...
- The roadmap is currently in **Phase 2 — Groovebox Core: Sequencer + Pads**. The scheduler and callback trigger path address matching Tracks at each step's MIDI note, with velocity-aware prepared zones. The touch sequencer grid, named pattern save/load, 16-pad kit editor (including WXI save copies), Instrument editors, and held-note sound Apply/Revert path exist and have selected two-board integration coverage. Pattern saves include hidden steps and groove settings; tempo and Track instruments remain session-owned. Melodic chord/gate lanes, MIDI-clock output, song/project persistence, physical panel integration and the full phase gate remain outstanding; parameter locks are implemented as described in the canonical [roadmap](docs/roadmap.md). Host tests and individual bench checks do not satisfy the whole phase gate.
