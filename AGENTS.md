...
5. **`docs/ui-architecture.md`**, **`docs/ui-design-constraints.md`**, **`docs/testing_guide.md`**, **`docs/performance_monitoring.md`**, **`docs/flashing.md`**, **`docs/logging.md`** — working guides for UI, testing, profiling, flashing and debug logging. Load the `lvgl9` project skill as well whenever designing, implementing, or reviewing `firmware/esp32/components/ui/` code.
...
- **The same code is built at more than one C++ standard, so target the lowest that will compile it.** The ESP32 device image is `gnu++2b` (pinned explicitly in `firmware/esp32/main/CMakeLists.txt` and `components/ui/CMakeLists.txt` — not inherited from ESP-IDF's default, so a toolchain bump cannot move it silently). **Everything else is C++17**: the Daisy device build (`firmware/daisy/CMakeLists.txt`, raised deliberately from the ARM toolchain's `gnu++14` default because shared logging state uses C++17 inline variables) and all three host test trees (`firmware/{daisy,esp32,shared}/tests/CMakeLists.txt`). Anything under `firmware/shared/` is compiled by all of them, so **shared code must be valid C++17** — `firmware/shared/config/logging_config.h` `static_assert`s on exactly that. Target-specific code may use its own target's standard.
...
- Full build/flash instructions (prerequisites, individual `idf.py`/`make` commands, debug probes) live in **`README.md`** and **`docs/flashing.md`** — don't duplicate them here. Quick reference: `make all` (both MCUs), `make esp32` / `make daisy` (individually), `make test` (host tests).
...
- **Firmware work happens inside the devcontainer — builds, tests, AND `git commit`** (`.devcontainer/Dockerfile` → image `wavex-devcontainer:latest`). **Hardware CAD is the exception:** run `hardware/Makefile` targets and KiCad/Python IPC tooling on the host with KiCad 10; see [`hardware/AGENTS.md`](hardware/AGENTS.md). Pre-commit and all commits remain container-only. The host toolchain is incomplete (no ESP-IDF env, `arm-none-eabi-gcc` present but the CMake toolchain file needs `CMAKE_C_COMPILER`/`CMAKE_CXX_COMPILER` set explicitly, which only the Daisy wrapper Makefile does correctly), and host-side hook runs leave stale-pathed build caches that then break container builds.
...
- **Continuation priority (2026-09-21):** the user authorized sampling from both codec input and internal mix, then arpeggiator, then global LFO editing, step-hold locks, eviction feedback and diagnostic counters ahead of capacity/SD remediation. Retain the 85.3713% finding and all physical/capacity/release gates.

- The roadmap is currently in **Phase 2 — Groovebox Core: Sequencer + Pads**. The scheduler and callback trigger path address matching Tracks at each step's MIDI note, with velocity-aware prepared zones. The touch sequencer grid, named Pattern save/load, 16-pad kit editor, Instrument editors and held-note sound Apply/Revert path have selected two-board integration coverage. Compact live MIDI admission preserves press identity; saved Instrument/Kit and Track polyphony controls include Mono held-key fallback. Melodic rows provide four note lanes, musical gates/holds and step/live recording. Pattern and Project persistence retain the lanes; tempo and Track instruments remain session-owned. Song/Project workflows, parameter locks and MIDI-clock output are implemented as described in the canonical [roadmap](docs/roadmap.md). Callback capacity, physical panel/MIDI integration and full phase gates remain open. Host tests and individual bench checks do not satisfy a whole phase gate.
- **2026-09-21 implementation update:** resident sample editing includes channel selection, stereo Snap/seam checks, playback crossfade and standalone Save/Save As. Instrument modulation includes oscillator mix and LFO 1/2 rate destinations; live Play-control lock recording captures onto the armed Track. UI contrast, header clipping, menu scrolling and diagnostic availability text have been corrected. These features have host and selected two-board checks; physical acceptance remains open in [hardware-validation.md](docs/hardware-validation.md).
- **Sampling/arp implementation (2026-09-21):** codec stereo/left/right and internal post-master capture, audition, WAV/sidecar Save and existing Pool assignment are implemented, as are the Instrument arpeggiator and session-global LFO controls. Held-step encoder locks, eviction notices and received-MIDI diagnostic counts are implemented. Selected board workflows do not close physical/listening, recovery, MIDI-clock or full-load gates (HV-030–HV-032). The latest short eight-Mono internal-recording/arp screen reached **86.5040%**, following the earlier **85.3713%** finding; preserve the user's continuation exception without claiming capacity acceptance.
...
- Commit each completed, validated task as part of finishing it, with its related documentation and changelog entries in the same commit. Do not accumulate completed tasks as uncommitted work; keep unrelated tasks in separate commits and preserve open hardware acceptance honestly.
