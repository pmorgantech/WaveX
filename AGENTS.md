...
- The roadmap is currently in **Phase 2 — Groovebox Core: Sequencer + Pads**. The scheduler and callback trigger path address matching Tracks at each step's MIDI note, with velocity-aware prepared zones. The touch sequencer grid and 16-pad kit editor (including WXI save copies) exist and have selected two-board integration coverage. Melodic chord/gate lanes, MIDI-clock output, parameter-lock application, pattern/song persistence, physical panel integration and the full phase gate remain outstanding. Host tests and individual bench checks do not satisfy the whole phase gate.
